-- Record the current analytics terminal application/schema compatibility contract.
--
...
select pg_advisory_xact_lock(
  hashtext('atehna:database-schema-contract:20260907.historical-orders-v6')
);
...
begin
  ...
  -- Retired analytics objects must be absent; their configuration is inert.
  if to_regclass('public.analytics_charts') is not null then raise exception 'Retired analytics table remains: analytics_charts'; end if;
  if to_regclass('public.analytics_chart_settings') is not null then raise exception 'Retired analytics table remains: analytics_chart_settings'; end if;
  if exists (select 1 from pg_proc join pg_namespace on pg_namespace.oid = pg_proc.pronamespace where pg_namespace.nspname = 'public' and proname = 'set_analytics_charts_updated_at') then raise exception 'Retired analytics function remains'; end if;
  if to_regclass('public.diagnostics_events') is null then raise exception 'Missing analytics table: diagnostics_events'; end if;
  if to_regclass('public.retired_configuration_archive') is null then raise exception 'Missing analytics table: retired_configuration_archive'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'id' and data_type = 'uuid' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: diagnostics_events.id'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'recorded_at' and data_type = 'timestamp with time zone' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: diagnostics_events.recorded_at'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'trace_id' and data_type = 'uuid' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: diagnostics_events.trace_id'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'context' and data_type = 'text' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: diagnostics_events.context'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'operation' and data_type = 'text' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: diagnostics_events.operation'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'kind' and data_type = 'text' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: diagnostics_events.kind'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'duration_ms' and data_type = 'double precision' and is_nullable = 'YES') then raise exception 'Incompatible analytics column: diagnostics_events.duration_ms'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'payload_bytes' and data_type = 'bigint' and is_nullable = 'YES') then raise exception 'Incompatible analytics column: diagnostics_events.payload_bytes'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'error' and data_type = 'boolean' and is_nullable = 'NO' and column_default = 'false') then raise exception 'Incompatible analytics column: diagnostics_events.error'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'error_code' and data_type = 'text' and is_nullable = 'YES') then raise exception 'Incompatible analytics column: diagnostics_events.error_code'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'phases_json' and data_type = 'jsonb' and is_nullable = 'NO' and column_default = '''{}''::jsonb') then raise exception 'Incompatible analytics column: diagnostics_events.phases_json'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'diagnostics_events' and column_name = 'details_json' and data_type = 'jsonb' and is_nullable = 'NO' and column_default = '''{}''::jsonb') then raise exception 'Incompatible analytics column: diagnostics_events.details_json'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'retired_configuration_archive' and column_name = 'source_table' and data_type = 'text' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: retired_configuration_archive.source_table'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'retired_configuration_archive' and column_name = 'source_key' and data_type = 'text' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: retired_configuration_archive.source_key'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'retired_configuration_archive' and column_name = 'captured_at' and data_type = 'timestamp with time zone' and is_nullable = 'NO' and column_default = 'now()') then raise exception 'Incompatible analytics column: retired_configuration_archive.captured_at'; end if;
  if not exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'retired_configuration_archive' and column_name = 'payload' and data_type = 'jsonb' and is_nullable = 'NO') then raise exception 'Incompatible analytics column: retired_configuration_archive.payload'; end if;
  if not exists (select 1 from pg_constraint where conrelid = 'public.diagnostics_events'::regclass and conname = 'diagnostics_events_details_json_check' and convalidated and pg_get_constraintdef(oid, true) = 'CHECK (jsonb_typeof(details_json) = ''object''::text)') then raise exception 'Analytics constraint mismatch: diagnostics_events_details_json_check'; end if;
  if not exists (select 1 from pg_constraint where conrelid = 'public.diagnostics_events'::regclass and conname = 'diagnostics_events_duration_check' and convalidated and pg_get_constraintdef(oid, true) = 'CHECK (duration_ms IS NULL OR duration_ms >= 0::double precision)') then raise exception 'Analytics constraint mismatch: diagnostics_events_duration_check'; end if;
  if not exists (select 1 from pg_constraint where conrelid = 'public.diagnostics_events'::regclass and conname = 'diagnostics_events_kind_check' and convalidated and pg_get_constraintdef(oid, true) = 'CHECK (kind = ANY (ARRAY[''route''::text, ''loader''::text, ''cache_miss''::text, ''invalidation''::text]))') then raise exception 'Analytics constraint mismatch: diagnostics_events_kind_check'; end if;
  if not exists (select 1 from pg_constraint where conrelid = 'public.diagnostics_events'::regclass and conname = 'diagnostics_events_payload_bytes_check' and convalidated and pg_get_constraintdef(oid, true) = 'CHECK (payload_bytes IS NULL OR payload_bytes >= 0)') then raise exception 'Analytics constraint mismatch: diagnostics_events_payload_bytes_check'; end if;
  if not exists (select 1 from pg_constraint where conrelid = 'public.diagnostics_events'::regclass and conname = 'diagnostics_events_phases_json_check' and convalidated and pg_get_constraintdef(oid, true) = 'CHECK (jsonb_typeof(phases_json) = ''object''::text)') then raise exception 'Analytics constraint mismatch: diagnostics_events_phases_json_check'; end if;
  if not exists (select 1 from pg_constraint where conrelid = 'public.retired_configuration_archive'::regclass and conname = 'retired_configuration_archive_payload_check' and convalidated and pg_get_constraintdef(oid, true) = 'CHECK (jsonb_typeof(payload) = ''object''::text)') then raise exception 'Analytics constraint mismatch: retired_configuration_archive_payload_check'; end if;
  if not exists (select 1 from pg_constraint where conrelid = 'public.retired_configuration_archive'::regclass and conname = 'retired_configuration_archive_source_check' and convalidated and pg_get_constraintdef(oid, true) = 'CHECK (source_table = ANY (ARRAY[''analytics_charts''::text, ''analytics_chart_settings''::text]))') then raise exception 'Analytics constraint mismatch: retired_configuration_archive_source_check'; end if;
  if not exists (select 1 from pg_index join pg_class on pg_class.oid = pg_index.indexrelid where relname = 'diagnostics_events_context_recorded_at_idx' and indisvalid and indisready and pg_get_indexdef(pg_index.indexrelid) = 'CREATE INDEX diagnostics_events_context_recorded_at_idx ON public.diagnostics_events USING btree (context, recorded_at)') then raise exception 'Analytics index mismatch: diagnostics_events_context_recorded_at_idx'; end if;
  if not exists (select 1 from pg_index join pg_class on pg_class.oid = pg_index.indexrelid where relname = 'diagnostics_events_error_recorded_at_idx' and indisvalid and indisready and pg_get_indexdef(pg_index.indexrelid) = 'CREATE INDEX diagnostics_events_error_recorded_at_idx ON public.diagnostics_events USING btree (recorded_at) WHERE error') then raise exception 'Analytics index mismatch: diagnostics_events_error_recorded_at_idx'; end if;
  if not exists (select 1 from pg_index join pg_class on pg_class.oid = pg_index.indexrelid where relname = 'diagnostics_events_pkey' and indisvalid and indisready and pg_get_indexdef(pg_index.indexrelid) = 'CREATE UNIQUE INDEX diagnostics_events_pkey ON public.diagnostics_events USING btree (id)') then raise exception 'Analytics index mismatch: diagnostics_events_pkey'; end if;
  if not exists (select 1 from pg_index join pg_class on pg_class.oid = pg_index.indexrelid where relname = 'diagnostics_events_recorded_at_idx' and indisvalid and indisready and pg_get_indexdef(pg_index.indexrelid) = 'CREATE INDEX diagnostics_events_recorded_at_idx ON public.diagnostics_events USING btree (recorded_at)') then raise exception 'Analytics index mismatch: diagnostics_events_recorded_at_idx'; end if;
  if not exists (select 1 from pg_index join pg_class on pg_class.oid = pg_index.indexrelid where relname = 'retired_configuration_archive_pkey' and indisvalid and indisready and pg_get_indexdef(pg_index.indexrelid) = 'CREATE UNIQUE INDEX retired_configuration_archive_pkey ON public.retired_configuration_archive USING btree (source_table, source_key)') then raise exception 'Analytics index mismatch: retired_configuration_archive_pkey'; end if;
end;
...
-- Additional price/stock invariants captured from the canonical local schema.
do $pricing_stock_contract$
declare
  requirements jsonb := $pricing_stock_requirements${"tables":["pricing_stock_history","pricing_stock_model"],"columns":[{"table":"catalog_item_variants","name":"work_minutes","dataType":"numeric","nullable":true},{"table":"catalog_item_variants","name":"other_costs","dataType":"numeric","nullable":true},{"table":"catalog_item_variants","name":"purchase_updated_at","dataType":"timestamp with time zone","nullable":true},{"table":"catalog_item_variants","name":"stock_revision","dataType":"bigint","nullable":false,"defaultEquals":"0"},{"table":"catalog_item_variants","name":"pricing_revision","dataType":"bigint","nullable":false,"defaultEquals":"0"},{"table":"pricing_stock_history","name":"id","dataType":"bigint","nullable":false,"defaultEquals":"nextval('pricing_stock_history_id_seq'::regclass)"},{"table":"pricing_stock_history","name":"occurred_at","dataType":"timestamp with time zone","nullable":false,"defaultEquals":"clock_timestamp()"},{"table":"pricing_stock_history","name":"entity_type","dataType":"text","nullable":false},{"table":"pricing_stock_history","name":"entity_id","dataType":"text","nullable":false},{"table":"pricing_stock_history","name":"item_id","dataType":"bigint","nullable":true},{"table":"pricing_stock_history","name":"sku","dataType":"text","nullable":true},{"table":"pricing_stock_history","name":"actor_id","dataType":"text","nullable":true},{"table":"pricing_stock_history","name":"actor_name","dataType":"text","nullable":true},{"table":"pricing_stock_history","name":"source","dataType":"text","nullable":false},{"table":"pricing_stock_history","name":"request_id","dataType":"text","nullable":true},{"table":"pricing_stock_history","name":"model_revision","dataType":"bigint","nullable":true},{"table":"pricing_stock_history","name":"before_json","dataType":"jsonb","nullable":false},{"table":"pricing_stock_history","name":"after_json","dataType":"jsonb","nullable":false},{"table":"pricing_stock_model","name":"key","dataType":"text","nullable":false},{"table":"pricing_stock_model","name":"revision","dataType":"bigint","nullable":false,"defaultEquals":"0"},{"table":"pricing_stock_model","name":"config_json","dataType":"jsonb","nullable":false},{"table":"pricing_stock_model","name":"updated_at","dataType":"timestamp with time zone","nullable":false,"defaultEquals":"now()"}],"constraints":[{"table":"catalog_item_variants","name":"catalog_variant_tdabc_inputs_check","type":"check","definitionEquals":"CHECK ((work_minutes IS NULL OR work_minutes >= 0::numeric AND work_minutes <= 99999999.9999) AND (other_costs IS NULL OR other_costs >= 0::numeric AND other_costs <= 9999999999.99) AND stock_revision >= 0 AND pricing_revision >= 0)"},{"table":"pricing_stock_history","name":"pricing_stock_history_after_json_check","type":"check","definitionEquals":"CHECK (jsonb_typeof(after_json) = 'object'::text)"},{"table":"pricing_stock_history","name":"pricing_stock_history_before_json_check","type":"check","definitionEquals":"CHECK (jsonb_typeof(before_json) = 'object'::text)"},{"table":"pricing_stock_history","name":"pricing_stock_history_entity_type_check","type":"check","definitionEquals":"CHECK (entity_type = ANY (ARRAY['variant'::text, 'model'::text]))"},{"table":"pricing_stock_model","name":"pricing_stock_model_config_json_check","type":"check","definitionEquals":"CHECK (jsonb_typeof(config_json) = 'object'::text)"},{"table":"pricing_stock_model","name":"pricing_stock_model_key_check","type":"check","definitionEquals":"CHECK (key = 'default'::text)"},{"table":"pricing_stock_model","name":"pricing_stock_model_revision_check","type":"check","definitionEquals":"CHECK (revision >= 0)"}],"functions":[{"name":"record_catalog_variant_pricing_stock","bodySha256":"82f9c48dce34261f77363ee6e4558c8fef901493b02c0673b9129307d1f0a782","language":"plpgsql","securityDefiner":false,"leakproof":false,"strict":false,"configuration":null,"volatility":"volatile","parallel":"unsafe","returns":"trigger","definitionIncludes":["pricing_stock_history","atehna.pricing_stock_audit","before_json","after_json"]},{"name":"track_catalog_variant_pricing_stock","bodySha256":"399a2999accb8ffea6ab155be1070bb809d7bcc848f31189cddf26cc59df9b3c","language":"plpgsql","securityDefiner":false,"leakproof":false,"strict":false,"configuration":null,"volatility":"volatile","parallel":"unsafe","returns":"trigger","definitionIncludes":["stock_revision","pricing_revision","purchase_updated_at","is distinct from"]}],"indexes":[{"table":"pricing_stock_history","name":"idx_pricing_stock_history_entity","definitionEquals":"CREATE INDEX idx_pricing_stock_history_entity ON public.pricing_stock_history USING btree (entity_type, entity_id, occurred_at DESC)","definitionIncludes":["pricing_stock_history","entity_id"]},{"table":"pricing_stock_history","name":"pricing_stock_history_pkey","definitionEquals":"CREATE UNIQUE INDEX pricing_stock_history_pkey ON public.pricing_stock_history USING btree (id)","definitionIncludes":["pricing_stock_history","unique"]},{"table":"pricing_stock_model","name":"pricing_stock_model_pkey","definitionEquals":"CREATE UNIQUE INDEX pricing_stock_model_pkey ON public.pricing_stock_model USING btree (key)","definitionIncludes":["pricing_stock_model","unique"]}],"triggers":[{"table":"catalog_item_variants","name":"catalog_variant_pricing_stock_revision","function":"track_catalog_variant_pricing_stock","definitionEquals":"CREATE TRIGGER catalog_variant_pricing_stock_revision BEFORE INSERT OR UPDATE ON catalog_item_variants FOR EACH ROW EXECUTE FUNCTION track_catalog_variant_pricing_stock()","definitionIncludes":["before","update","for each row","track_catalog_variant_pricing_stock"]},{"table":"catalog_item_variants","name":"catalog_variant_pricing_stock_history","function":"record_catalog_variant_pricing_stock","definitionEquals":"CREATE TRIGGER catalog_variant_pricing_stock_history AFTER UPDATE ON catalog_item_variants FOR EACH ROW EXECUTE FUNCTION record_catalog_variant_pricing_stock()","definitionIncludes":["after","update","for each row","record_catalog_variant_pricing_stock"]}],"settings":[{"table":"pricing_stock_model","key":"default","jsonField":"formulaVersion","jsonType":"number"},{"table":"pricing_stock_model","key":"default","jsonField":"formula","jsonType":"string"},{"table":"pricing_stock_model","key":"default","jsonField":"parameters","jsonType":"object"}]}$pricing_stock_requirements$::jsonb;
  requirement jsonb; actual record; setting_type text;
begin
  for requirement in select value from jsonb_array_elements(requirements->'tables') loop
    if to_regclass('public.' || (requirement #>> '{}')) is null then raise exception 'Missing pricing table: %', requirement; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'columns') loop
    select data_type,is_nullable,column_default into actual from information_schema.columns where table_schema='public' and table_name=requirement->>'table' and column_name=requirement->>'name';
    if not found or actual.data_type <> requirement->>'dataType' or (actual.is_nullable='YES') <> (requirement->>'nullable')::boolean or (requirement ? 'defaultEquals' and actual.column_default is distinct from requirement->>'defaultEquals') then raise exception 'Invalid pricing column: %', requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'constraints') loop
    select k.convalidated,k.contype,pg_get_constraintdef(k.oid,true) as definition into actual from pg_constraint k join pg_class c on c.oid=k.conrelid join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname=requirement->>'table' and k.conname=requirement->>'name';
    if not found or not actual.convalidated or actual.contype <> (case when requirement->>'type'='check' then 'c'::"char" else 'f'::"char" end) or actual.definition <> requirement->>'definitionEquals' then raise exception 'Invalid pricing constraint: %',requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'functions') loop
    select p.*,l.lanname,pg_get_function_result(p.oid) as result_type into actual from pg_proc p join pg_namespace n on n.oid=p.pronamespace join pg_language l on l.oid=p.prolang where n.nspname='public' and p.proname=requirement->>'name' and p.pronargs=0;
    if not found or encode(sha256(convert_to(replace(actual.prosrc,E'\r',''),'UTF8')),'hex') <> requirement->>'bodySha256' or actual.prosecdef or actual.proleakproof or actual.proisstrict or actual.proconfig is not null or actual.provolatile <> 'v' or actual.proparallel <> 'u' or actual.lanname <> 'plpgsql' or actual.result_type <> 'trigger' then raise exception 'Invalid pricing routine: %',requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'indexes') loop
    select i.indexdef,x.indisvalid,x.indisready into actual from pg_indexes i join pg_class c on c.relname=i.indexname join pg_namespace n on n.oid=c.relnamespace and n.nspname=i.schemaname join pg_index x on x.indexrelid=c.oid where i.schemaname='public' and i.tablename=requirement->>'table' and i.indexname=requirement->>'name';
    if not found or not actual.indisvalid or not actual.indisready or actual.indexdef <> requirement->>'definitionEquals' then raise exception 'Invalid pricing index: %',requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'triggers') loop
    select pg_get_triggerdef(t.oid,true) as definition,t.tgenabled into actual from pg_trigger t join pg_class c on c.oid=t.tgrelid join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname=requirement->>'table' and t.tgname=requirement->>'name';
    if not found or actual.tgenabled <> 'O' or actual.definition <> requirement->>'definitionEquals' then raise exception 'Invalid pricing trigger: %',requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'settings') loop
    execute format('select jsonb_typeof(config_json->%L) from %I where key=$1',requirement->>'jsonField',requirement->>'table') into setting_type using requirement->>'key';
    if setting_type is distinct from requirement->>'jsonType' then raise exception 'Invalid pricing setting: %',requirement->>'jsonField'; end if;
  end loop;
end;
$pricing_stock_contract$;

-- Additional historical-order invariants captured from the canonical local schema.
do $historical_orders_contract$
declare
  requirements jsonb := $historical_orders_requirements${"tables":["order_historical_changes","business_analytics_settings"],"columns":[{"table":"business_analytics_settings","name":"key","dataType":"text","nullable":false},{"table":"business_analytics_settings","name":"quote_go_live_date","dataType":"date","nullable":true},{"table":"business_analytics_settings","name":"revision","dataType":"bigint","nullable":false,"defaultEquals":"0"},{"table":"business_analytics_settings","name":"updated_at","dataType":"timestamp with time zone","nullable":false,"defaultEquals":"now()"},{"table":"deleted_archive_entries","name":"expires_at","dataType":"timestamp with time zone","nullable":true,"defaultEquals":null},{"table":"order_historical_changes","name":"id","dataType":"bigint","nullable":false,"defaultEquals":"nextval('order_historical_changes_id_seq'::regclass)"},{"table":"order_historical_changes","name":"order_id","dataType":"bigint","nullable":false},{"table":"order_historical_changes","name":"revision","dataType":"bigint","nullable":false},{"table":"order_historical_changes","name":"changed_at","dataType":"timestamp with time zone","nullable":false,"defaultEquals":"clock_timestamp()"},{"table":"order_historical_changes","name":"actor_id","dataType":"text","nullable":false},{"table":"order_historical_changes","name":"before_json","dataType":"jsonb","nullable":false},{"table":"order_historical_changes","name":"after_json","dataType":"jsonb","nullable":false},{"table":"orders","name":"entry_source","dataType":"text","nullable":true},{"table":"orders","name":"is_historical","dataType":"boolean","nullable":false,"defaultEquals":"false"},{"table":"orders","name":"original_reference_system","dataType":"text","nullable":true},{"table":"orders","name":"original_reference","dataType":"text","nullable":true},{"table":"orders","name":"recorded_at","dataType":"timestamp with time zone","nullable":false,"defaultEquals":"clock_timestamp()"},{"table":"orders","name":"historical_fulfilled_at","dataType":"timestamp with time zone","nullable":true},{"table":"orders","name":"historical_payment_at","dataType":"timestamp with time zone","nullable":true},{"table":"orders","name":"historical_revision","dataType":"bigint","nullable":false,"defaultEquals":"0"},{"table":"orders","name":"archived_at","dataType":"timestamp with time zone","nullable":true}],"constraints":[{"table":"business_analytics_settings","name":"business_analytics_settings_key_check","type":"check","definitionEquals":"CHECK (key = 'default'::text)"},{"table":"business_analytics_settings","name":"business_analytics_settings_revision_check","type":"check","definitionEquals":"CHECK (revision >= 0)"},{"table":"order_historical_changes","name":"order_historical_changes_check","type":"check","definitionEquals":"CHECK (jsonb_typeof(before_json) = 'object'::text AND jsonb_typeof(after_json) = 'object'::text)"},{"table":"order_historical_changes","name":"order_historical_changes_order_id_fkey","type":"foreign_key","definitionEquals":"FOREIGN KEY (order_id) REFERENCES orders(id)"},{"table":"order_historical_changes","name":"order_historical_changes_revision_check","type":"check","definitionEquals":"CHECK (revision > 0)"},{"table":"orders","name":"orders_contract_state_evidence_check","type":"check","definitionEquals":"CHECK (contract_status = 'pending_seller_acceptance'::text AND contract_accepted_at IS NULL AND contract_accepted_actor_type IS NULL AND contract_accepted_actor_id IS NULL AND contract_acceptance_evidence_json IS NULL AND contract_rejected_at IS NULL AND contract_rejected_actor_type IS NULL AND contract_rejected_actor_id IS NULL AND contract_rejection_reason IS NULL AND contract_rejection_evidence_json IS NULL AND committed_at IS NULL OR contract_status = 'accepted'::text AND (is_historical OR contract_accepted_at IS NOT NULL) AND contract_accepted_actor_type IS NOT NULL AND contract_acceptance_evidence_json IS NOT NULL AND contract_rejected_at IS NULL AND contract_rejected_actor_type IS NULL AND contract_rejected_actor_id IS NULL AND contract_rejection_reason IS NULL AND contract_rejection_evidence_json IS NULL AND (is_historical OR committed_at IS NOT NULL) OR contract_status = 'rejected'::text AND contract_accepted_at IS NULL AND contract_accepted_actor_type IS NULL AND contract_accepted_actor_id IS NULL AND contract_acceptance_evidence_json IS NULL AND contract_rejected_at IS NOT NULL AND contract_rejected_actor_type IS NOT NULL AND contract_rejection_evidence_json IS NOT NULL AND committed_at IS NULL)"},{"table":"orders","name":"orders_entry_source_check","type":"check","definitionEquals":"CHECK (entry_source IS NULL OR (entry_source = ANY (ARRAY['website'::text, 'manual'::text])))"},{"table":"orders","name":"orders_historical_guard_check","type":"check","definitionEquals":"CHECK (historical_revision >= 0 AND (NOT is_historical OR NOT entry_source IS DISTINCT FROM 'manual'::text AND NOT stock_enforcement_applied))"},{"table":"orders","name":"orders_original_reference_check","type":"check","definitionEquals":"CHECK ((original_reference_system IS NULL) = (original_reference IS NULL) AND (original_reference_system IS NULL OR length(btrim(original_reference_system)) >= 1 AND length(btrim(original_reference_system)) <= 80) AND (original_reference IS NULL OR length(btrim(original_reference)) >= 1 AND length(btrim(original_reference)) <= 200) AND (NOT is_historical OR is_draft OR original_reference IS NOT NULL))"}],"functions":[{"name":"capture_order_analytics_snapshot","bodySha256":"8a8fc134af45aa3c2c5021398308966055a037a076c54c78a10308c6c57a2436","language":"plpgsql","securityDefiner":false,"leakproof":false,"strict":false,"configuration":null,"volatility":"volatile","parallel":"unsafe","returns":"trigger","definitionIncludes":["is_historical","historical_fulfilled_at","historical_payment_at"]},{"name":"protect_historical_change_log","bodySha256":"b5dd3757b0e9cebb3126eb83f04e831314947b42da18ece0a90daabe1fb2d906","language":"plpgsql","securityDefiner":false,"leakproof":false,"strict":false,"configuration":null,"volatility":"volatile","parallel":"unsafe","returns":"trigger","definitionIncludes":["raise exception","Historical order"]},{"name":"protect_order_audit_history","bodySha256":"a29f9722e45c9f66c5673c3a259b237a4e0b9c4b40d534d580ee309b3256fb37","language":"plpgsql","securityDefiner":false,"leakproof":false,"strict":false,"configuration":null,"volatility":"volatile","parallel":"unsafe","returns":"trigger","definitionIncludes":["entity_type","'order'","'pdf'"]},{"name":"protect_order_entry_evidence","bodySha256":"7b42b7560dd16d83c34318116885ef81901acc3962b69f98682ff5d038a1eb9a","language":"plpgsql","securityDefiner":false,"leakproof":false,"strict":false,"configuration":null,"volatility":"volatile","parallel":"unsafe","returns":"trigger","definitionIncludes":["recorded_at","is_historical","entry_source"]},{"name":"reject_historical_order_stock_hold","bodySha256":"590affce0280f84c7f05d0106a314baf2ee7d851e44507a51a16b610cb16ebe8","language":"plpgsql","securityDefiner":false,"leakproof":false,"strict":false,"configuration":null,"volatility":"volatile","parallel":"unsafe","returns":"trigger","definitionIncludes":["order_id","is_historical"]}],"indexes":[{"table":"business_analytics_settings","name":"business_analytics_settings_pkey","definitionEquals":"CREATE UNIQUE INDEX business_analytics_settings_pkey ON public.business_analytics_settings USING btree (key)","definitionIncludes":["business_analytics_settings","btree"]},{"table":"order_historical_changes","name":"order_historical_changes_order_id_revision_key","definitionEquals":"CREATE UNIQUE INDEX order_historical_changes_order_id_revision_key ON public.order_historical_changes USING btree (order_id, revision)","definitionIncludes":["order_historical_changes","btree"]},{"table":"order_historical_changes","name":"order_historical_changes_pkey","definitionEquals":"CREATE UNIQUE INDEX order_historical_changes_pkey ON public.order_historical_changes USING btree (id)","definitionIncludes":["order_historical_changes","btree"]},{"table":"orders","name":"orders_archived_at_idx","definitionEquals":"CREATE INDEX orders_archived_at_idx ON public.orders USING btree (archived_at) WHERE (archived_at IS NOT NULL)","definitionIncludes":["orders","btree"]},{"table":"orders","name":"orders_entry_source_created_idx","definitionEquals":"CREATE INDEX orders_entry_source_created_idx ON public.orders USING btree (entry_source, created_at)","definitionIncludes":["orders","btree"]},{"table":"orders","name":"orders_original_reference_unique","definitionEquals":"CREATE UNIQUE INDEX orders_original_reference_unique ON public.orders USING btree (lower(btrim(original_reference_system)), lower(btrim(original_reference))) WHERE (original_reference IS NOT NULL)","definitionIncludes":["orders","btree"]}],"triggers":[{"table":"audit_events","name":"audit_events_protect_order_history","function":"protect_order_audit_history","definitionEquals":"CREATE TRIGGER audit_events_protect_order_history BEFORE DELETE ON audit_events FOR EACH ROW EXECUTE FUNCTION protect_order_audit_history()","definitionIncludes":["before","for each row","protect_order_audit_history"]},{"table":"order_historical_changes","name":"order_historical_changes_immutable","function":"protect_historical_change_log","definitionEquals":"CREATE TRIGGER order_historical_changes_immutable BEFORE DELETE OR UPDATE ON order_historical_changes FOR EACH ROW EXECUTE FUNCTION protect_historical_change_log()","definitionIncludes":["before","for each row","protect_historical_change_log"]},{"table":"order_stock_holds","name":"order_stock_holds_reject_historical","function":"reject_historical_order_stock_hold","definitionEquals":"CREATE TRIGGER order_stock_holds_reject_historical BEFORE INSERT OR UPDATE ON order_stock_holds FOR EACH ROW EXECUTE FUNCTION reject_historical_order_stock_hold()","definitionIncludes":["before","for each row","reject_historical_order_stock_hold"]},{"table":"orders","name":"orders_protect_entry_evidence","function":"protect_order_entry_evidence","definitionEquals":"CREATE TRIGGER orders_protect_entry_evidence BEFORE INSERT OR UPDATE ON orders FOR EACH ROW EXECUTE FUNCTION protect_order_entry_evidence()","definitionIncludes":["before","for each row","protect_order_entry_evidence"]}],"settings":[],"requiredRows":[{"table":"business_analytics_settings","key":"default"}]}$historical_orders_requirements$::jsonb;
  requirement jsonb; actual record; setting_type text;
begin
  for requirement in select value from jsonb_array_elements(requirements->'tables') loop
    if to_regclass('public.' || (requirement #>> '{}')) is null then raise exception 'Missing historical table: %', requirement; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'columns') loop
    select data_type,is_nullable,column_default into actual from information_schema.columns where table_schema='public' and table_name=requirement->>'table' and column_name=requirement->>'name';
    if not found or actual.data_type <> requirement->>'dataType' or (actual.is_nullable='YES') <> (requirement->>'nullable')::boolean or (requirement ? 'defaultEquals' and actual.column_default is distinct from requirement->>'defaultEquals') then raise exception 'Invalid historical column: %', requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'constraints') loop
    select k.convalidated,k.contype,pg_get_constraintdef(k.oid,true) as definition into actual from pg_constraint k join pg_class c on c.oid=k.conrelid join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname=requirement->>'table' and k.conname=requirement->>'name';
    if not found or not actual.convalidated or actual.contype <> (case when requirement->>'type'='check' then 'c'::"char" else 'f'::"char" end) or actual.definition <> requirement->>'definitionEquals' then raise exception 'Invalid historical constraint: %',requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'functions') loop
    select p.*,l.lanname,pg_get_function_result(p.oid) as result_type into actual from pg_proc p join pg_namespace n on n.oid=p.pronamespace join pg_language l on l.oid=p.prolang where n.nspname='public' and p.proname=requirement->>'name' and p.pronargs=0;
    if not found or encode(sha256(convert_to(replace(actual.prosrc,E'\r',''),'UTF8')),'hex') <> requirement->>'bodySha256' or actual.prosecdef or actual.proleakproof or actual.proisstrict or actual.proconfig is not null or actual.provolatile <> 'v' or actual.proparallel <> 'u' or actual.lanname <> 'plpgsql' or actual.result_type <> 'trigger' then raise exception 'Invalid historical routine: %',requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'indexes') loop
    select i.indexdef,x.indisvalid,x.indisready into actual from pg_indexes i join pg_class c on c.relname=i.indexname join pg_namespace n on n.oid=c.relnamespace and n.nspname=i.schemaname join pg_index x on x.indexrelid=c.oid where i.schemaname='public' and i.tablename=requirement->>'table' and i.indexname=requirement->>'name';
    if not found or not actual.indisvalid or not actual.indisready or actual.indexdef <> requirement->>'definitionEquals' then raise exception 'Invalid historical index: %',requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'triggers') loop
    select pg_get_triggerdef(t.oid,true) as definition,t.tgenabled into actual from pg_trigger t join pg_class c on c.oid=t.tgrelid join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname=requirement->>'table' and t.tgname=requirement->>'name';
    if not found or actual.tgenabled <> 'O' or actual.definition <> requirement->>'definitionEquals' then raise exception 'Invalid historical trigger: %',requirement->>'name'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'requiredRows') loop
    execute format('select count(*) from public.%I where key=$1',requirement->>'table') into setting_type using requirement->>'key';
    if setting_type is distinct from '1' then raise exception 'Missing required settings row: %',requirement->>'table'; end if;
  end loop;
  for requirement in select value from jsonb_array_elements(requirements->'settings') loop
    execute format('select jsonb_typeof(config_json->%L) from %I where key=$1',requirement->>'jsonField',requirement->>'table') into setting_type using requirement->>'key';
    if setting_type is distinct from requirement->>'jsonType' then raise exception 'Invalid historical setting: %',requirement->>'jsonField'; end if;
  end loop;
end;
$historical_orders_contract$;
...
begin
  if exists (
    select 1
     ...
     where contract_id = '20260907.historical-orders-v6'
       and contract_sha256 <>
          '2f9d3f55493eb28a40d6b16f025e8b40ca482c5d0d87e1023c3ab72d64b074af'
  ) then
    raise exception
      'Schema contract 20260907.historical-orders-v6 already exists with another checksum';
  end if;
end;
$contract_identity$;
