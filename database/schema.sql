...
create unique index admin_auth_user_singleton on admin_auth_user ((true));
...
create index admin_auth_session_user_idx on admin_auth_session ("userId");
create index admin_auth_session_expiry_idx on admin_auth_session ("expiresAt");
...
create table admin_session_policy (
...
insert into admin_session_policy (id, max_lifetime_days, idle_timeout_minutes) values (1, 30, 30);
...
create index admin_login_attempt_window_idx on admin_login_attempt (window_start);
...
create table catalog_item_variants (
  ...
  constraint catalog_variant_tdabc_inputs_check check (
...
create table analytics_geography_state (
...
create table analytics_geography_backfill (
...
create table pricing_stock_model (
...
create table pricing_stock_history (
...
create index idx_pricing_stock_history_entity on pricing_stock_history(entity_type, entity_id, occurred_at desc);

create function track_catalog_variant_pricing_stock()
returns trigger language plpgsql as $function$
begin
  if tg_op = 'INSERT' then
...
create function record_catalog_variant_pricing_stock()
returns trigger language plpgsql as $function$
