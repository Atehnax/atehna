...
For dimension-sensitive products, keep real photography and append precise technical SVG illustrations. Use the reviewed generator and media-only synchronization workflow in docs/catalog-import/dimension-sketches-2026-09.md. Create one separate sketch for every configured variant with verified measurements, including variants sharing a size but differing in colour, material or model. Assign each sketch only to that variant and show it only when that variant is selected. Replace older combined overviews through the managed synchronization workflow. Prefer explicit selectable product measurements; never substitute shipping dimensions, grams, package sizes or measuring capacity. Unknown measurements remain unspecified. Named colours may be schematic and must be labelled as such. Keep generated imageType markers through admin edits, display whole diagrams with contain sizing, and provide access to the full-resolution SVG. Regenerate and review sketches after dimension or variant changes.

Technical sketches must use a square canvas, minimal composition and large readable text. All numbers and labels stay horizontal, including dimensions of vertical or diagonal edges; use extension lines instead of rotating text. Use a white 1800 × 1800 canvas, slate outlines, arrow dimension lines and the supplied clean top/side-view reference. Show a separate thickness profile when known, noting any enlargement for readability. Do not combine variants into small overview grids or swatch sheets; keep each image independently readable.
