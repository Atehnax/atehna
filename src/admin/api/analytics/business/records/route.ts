...
export async function GET(request: Request) {
  ...
  try { const params = new URL(request.url).searchParams; const csv = params.get('format') === 'csv'; const data = await fetchBusinessRecords(params, csv); return csv ? new Response(businessRecordsCsv(data.records), { headers: { 'content-type': 'text/csv; charset=utf-8', 'content-disposition': 'attachment; filename="atehna-poslovna-analitika.csv"', 'cache-control': 'private, no-store' } }) : NextResponse.json(data, { headers: { 'cache-control': 'private, no-store' } }); }
