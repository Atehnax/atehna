...
import { eur, numeric, formatAnalyticsRecordDate } from '../../lib/formatting';
...
import { getPaymentLabel } from '@/shared/domain/order/paymentStatus';
import { getStatusLabel } from '@/shared/domain/order/orderStatus';
import { DataTable, type Drill } from './BusinessChart';


function drillLabel(key: string, value: string): string {
  const labels: Record<string, string> = { kind: 'Zapisi', basis: 'Osnova', date: 'Datum', customerType: 'Tip naročnika', source: 'Potek', entrySource: 'Način vnosa', history: 'Zgodovina', status: 'Status', min: 'Spodnja meja', max: 'Zgornja meja', inclusiveMax: 'Vključena zgornja meja', customerKey: 'Naročnik', orderId: 'ID naročila', productKey: 'Artikel', cohort: 'Kohorta', cohortMonth: 'Mesec kohorte', topCustomerCount: 'Prvih naročnikov', lorenzPopulation: 'Delež naročnikov', municipalityId: 'Občina', regionId: 'Regija' };
  const values: Record<string, string> = { orders: 'Naročila', quotes: 'Ponudbe', requests: 'Povpraševanja', issued: 'Prvič izdane ponudbe', historical: 'Zgodovinska', current: 'Tekoča', website: 'Splet', manual: 'Ročno', activity: 'Oddana naročila', paid: 'Plačana naročila', 'paid-net': 'Plačano po potrjenih vračilih', realised: 'Realizirana naročila', lorenz: 'Plačano blago pred vračili', weight: 'Dejanska masa (kg)', response: 'Odzivni čas (ure)', decision: 'Čas do sprejema (ure)', direct: 'Neposredno', quote: 'Iz ponudbe', true: 'Da', false: 'Ne', unknown: 'Neznano' };
...
export default function BusinessRecords({ query, drill, onClose }: { query: string; drill: Drill; onClose: () => void }) {
  ...
  return <dialog ref={dialog} onCancel={onClose} className="m-auto max-h-[90vh] w-[min(1000px,95vw)] rounded-2xl border border-slate-200 p-5 text-slate-900 shadow-xl backdrop:bg-slate-950/40">
    <div className="mb-4 flex items-center justify-between gap-4"><div><h2 className="text-lg font-semibold">Pripadajoči {drill.kind === 'quotes' || drill.kind === 'requests' ? 'ponudbeni primeri' : 'zapisi naročil'}</h2><p className="mt-1 text-xs text-slate-500">Enaka populacija in filtri kot izbrani graf.</p></div><button type="button" onClick={onClose} className="rounded-lg border px-3 py-2 text-sm">Zapri</button></div>
    ...
    {error ? <p role="alert" className="py-8 text-sm text-red-700">{error}</p> : !data ? <p role="status" className="py-8 text-sm text-slate-500">Nalaganje zapisov …</p> : <>
      <DataTable columns={['Številka', 'Datum', 'Naročnik', 'Tip', 'Status', 'Plačilo', 'Potek', 'Način vnosa', 'Zgodovina', data?.valueUnit === 'h' ? 'Čas (ure)' : data?.valueUnit === 'kg' ? 'Dejanska masa (kg)' : 'Vrednost brez DDV (EUR)']} rows={data.records.map(record => ({ href: record.href, values: [record.number, formatAnalyticsRecordDate(record.date), record.customerName, record.customerType === 'unknown' ? 'Neznano' : getCustomerTypeLabel(record.customerType), drill.kind === 'quotes' || drill.kind === 'requests' ? ({ issued: 'Izdana', accepted: 'Sprejeta', ordered: 'Naročena', declined: 'Zavrnjena', expired: 'Potekla', preparation: 'V pripravi', received: 'Prejeta' }[record.status] ?? record.status) : getStatusLabel(record.status), record.paymentStatus ? getPaymentLabel(record.paymentStatus) : '—', record.source === 'quote' ? 'Iz ponudbe' : 'Neposredno', record.entrySource === 'website' ? 'Splet' : record.entrySource === 'manual' ? 'Ročno' : 'Neznano', record.isHistorical === undefined ? '—' : record.isHistorical ? 'Zgodovinsko' : 'Tekoče', record.value == null ? '—' : data.valueUnit === 'h' || data.valueUnit === 'kg' ? numeric(record.value) + ' ' + data.valueUnit : eur(record.value)] }))} />
