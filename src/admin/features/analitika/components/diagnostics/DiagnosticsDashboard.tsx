...
const bytes = (value: number | null) => value == null || !Number.isFinite(value) ? '—' : value >= 1048576 ? numeric(value / 1048576) + ' MB' : value >= 1024 ? numeric(value / 1024) + ' kB' : numeric(value, 0) + ' B';
...
export default function DiagnosticsDashboard() {
  ...
  return <div className="space-y-4">
    <section className={adminAnalyticsPanelClassName}>
      <div className="flex flex-wrap items-center justify-between gap-3"><AdminPeriodSelector value={search.get('window') ?? '15m'} onChange={window => update({ window })} options={Object.keys(DIAGNOSTIC_WINDOWS).map(value => ({ value, label: windows[value] }))} ariaLabel="Diagnostično časovno okno" /><div className="flex items-center gap-3 text-xs"><button type="button" className={buttonTokenClasses.control} onClick={() => { if (search.has('asOf')) update({ asOf: null }); else setRevision(value => value + 1); }}>Osveži meritve</button><a className="text-blue-700 underline" href={'/api/admin/analytics/diagnostics?' + exportParams}>Izvoz CSV</a></div></div>
      ...
      {data && <p className="mt-3 text-[11px] text-slate-500">{dateLabel(data.filters.start)} – {dateLabel(data.asOf)} · Europe/Ljubljana · interval: {numeric(data.filters.bucketMinutes, 0)} min</p>}
    </section>
    {error ? <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}<button className="ml-3 underline" onClick={() => setRevision(value => value + 1)}>Poskusi znova</button></div> : loading ? <p role="status" className="p-8 text-sm text-slate-500">Nalaganje diagnostičnih meritev …</p> : data && <>
      <p className="text-xs leading-relaxed text-slate-500">Meritve se zapisujejo v podatkovno bazo in hranijo sedem dni. {data.coverage.firstRecordedAt ? 'Prva ohranjena meritev: ' + dateLabel(data.coverage.firstRecordedAt) + '.' : 'Zabeleženih meritev še ni.'} {!data.coverage.completeWindow && 'Za del izbranega okna zgodovina še ni na voljo.'} Prikazane so operacije, pri katerih zbiramo meritve; seznam ne zajema vseh omrežnih zahtev.</p>
      <div className="grid grid-cols-1 gap-[14px] sm:grid-cols-2 xl:grid-cols-5">{[
        ['Izbrane meritve', numeric(data.summary.observations, 0), 'Vsi zapisi, ki ustrezajo izbranim filtrom'],
        ['Napake', numeric(data.summary.errors, 0), percent(data.summary.errorRate) + ' izbranih meritev'],
        ['p95 trajanja', data.summary.p95Ms == null ? '—' : numeric(data.summary.p95Ms) + ' ms', '95. percentil veljavnih trajanj'],
        ['Zahteve', numeric(data.summary.routes, 0), 'Zaključene merjene zahteve'],
        ['Merjen prenos', data.summary.payloadMeasured ? bytes(data.summary.payloadBytes) : '—', formatSlCount(data.summary.payloadMeasured, { one: 'meritev', two: 'meritvi', few: 'meritve', other: 'meritev' }) + '; ocena serializiranega izhoda']
      ].map(([label, value, hint]) => <AdminAnalyticsMetricCard key={label} title={label} metric={value}>{hint}</AdminAnalyticsMetricCard>)}</div>
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <BusinessChart title="Trajanje operacij" description="Povprečje in p95 vseh izmerjenih trajanj v časovnem intervalu. Če ni meritev, točka manjka." data={[
