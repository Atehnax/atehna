...
export default function AdminArticlesWorkspace({ view, children }: { view: 'list' | 'pricing-stock' | 'suppliers'; children?: ReactNode }) {
  ...
  const navigate = (next: string) => {
    ...
    const run = () => { const params = new URLSearchParams(window.location.search); if (next !== 'list') params.set('view', next); else params.delete('view'); startTransition(() => router.push('/admin/artikli' + (params.size ? '?' + params : ''), { scroll: false })); };
  ...
  return <ArticleNavigationGuardContext.Provider value={register}>
    <div aria-busy={pending} className="min-w-0 space-y-3">
      <EuiTabs value={view} onChange={navigate} ariaLabel="Pogledi artiklov" idPrefix="articles" tabs={[{ value: 'list', label: 'Seznam artiklov', panelId: 'articles-list' }, { value: 'pricing-stock', label: 'Razlika v ceni', panelId: 'articles-pricing-stock' }, { value: 'suppliers', label: 'Dobavitelji', panelId: 'articles-suppliers' }]} />
