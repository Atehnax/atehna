        ...
        <Table className="w-full table-fixed text-[12px]">
            <THead>
              <TR>
                ...
                <TH className="w-[4%] whitespace-nowrap text-center">
                  <div className="relative inline-flex items-center gap-1">
                ...
                <TH className="w-[5%] whitespace-nowrap text-right">Akcijska cena</TH>
                <TH className="w-[2%] whitespace-nowrap text-right">Zaloga</TH>
                <TH className="w-[2%] whitespace-nowrap text-center">Min/nar.</TH>
                <TH className="w-[77px] whitespace-nowrap px-0 text-center">
                  <div className="relative inline-flex items-center gap-1">
                ...
                <TH className="w-[69px] whitespace-nowrap px-0 text-center">Opombe</TH>
                <TH className="w-12 text-center">Uredi</TH>
              </TR>
            </THead>
            <tbody>
                ...
                return (
                  <Fragment key={family.id}>
                    <tr className={`border-t border-slate-100 bg-white ${adminTableRowToneClasses.hover}`} data-edit-scope={`family:${family.id}`}>
                      ...
                      <td className="w-[77px] px-0 py-3 text-center"><div className="inline-flex justify-end pr-[5px]"><ActiveStateChip active={familyDraft.active} editable={isEditingFamily} editScope={`family:${family.id}`} chipClassName="!min-w-[84px] !text-[11px]" onChange={(next) => setFamilyDrafts((current) => ({ ...current, [family.id]: { ...familyDraft, active: next } }))} /></div></td>
                      <td className="w-[69px] px-0 py-3 text-center">
                        {isEditingFamily
                          ? <div className="inline-flex justify-start pl-[5px]"><NoteTagChip value={familyDraft.note} editable allowEmpty editScope={`family:${family.id}`} chipClassName="!min-w-[84px] !text-[11px]" placeholderLabel="Opombe" onChange={(next) => setFamilyDrafts((current) => ({ ...current, [family.id]: { ...familyDraft, note: next as NoteValue } }))} /></div>
                          : <div className="inline-flex justify-start pl-[5px]"><NoteTagChip value={(family.notes?.trim() as NoteValue) || ''} editable={false} allowEmpty editScope={`family:${family.id}`} chipClassName="!min-w-[84px] !text-[11px]" placeholderLabel="Opombe" onChange={() => {}} /></div>}
                      </td>
                      <td className="w-12 px-2 py-3 text-center"><RowActionsDropdown label={`Možnosti za ${family.name}`} items={[{ key: 'quick-edit', label: 'Hitro urejanje', icon: <PencilIcon />, onSelect: () => startFamilyEdit(family, visibleVariants) }, { key: 'save', label: 'Shrani', icon: <SaveIcon />, disabled: !isEditingFamily, onSelect: () => { void saveFamilyEdit(family, visibleVariants); } }, { key: 'edit', label: 'Uredi', onSelect: () => router.push(`/admin/artikli/${encodeURIComponent(family.slug || family.id)}`) }]} /></td>
                    </tr>
