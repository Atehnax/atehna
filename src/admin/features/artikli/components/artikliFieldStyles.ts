'use client';

export {
  adminArticleNameInputClassName as articleNameInputClassName,
  adminCompactIconFieldInputClassName as compactSideInputClassName,
  adminCompactIconFieldShellClassName as compactSideInputWrapClassName,
  adminCompactTableAdornmentClassName as compactTableAdornmentClassName,
  adminCompactTableAlignedInputClassName as compactTableAlignedInputClassName,
  adminCompactTableAlignedTextInputClassName as compactTableAlignedTextInputClassName,
  adminCompactTableValueUnitShellClassName as compactTableValueUnitShellClassName,
  adminNumberInputClassName as numberInputClass,
  adminTopBarArticleNameInputClassName as topBarArticleNameInputClassName
} from '@/shared/ui/admin-controls/adminCompactFieldStyles';
