...
export const InlineStatusToggle = ({
  ...
  <button
    ...
    className="relative inline-flex h-7 w-[58px] items-center rounded-full border-2 border-[#3b4754] bg-[#f2f2f1] transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#3e67d6]/50 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
  >
    <span aria-hidden="true" className={`pointer-events-none absolute top-1/2 -translate-y-1/2 transition-all duration-200 ${checked ? 'left-1.5 text-[#1fa56a]' : 'right-1.5 text-[#df4f67]'}`}>
      {checked ? (
      ...
      <span
        aria-hidden="true"
      className={`absolute left-[-1px] top-1/2 z-10 h-6 w-6 -translate-y-1/2 rounded-full border border-[#37424f] bg-[#4f5b68] shadow-[inset_0_1px_2px_rgba(255,255,255,0.08),inset_0_-3px_6px_rgba(0,0,0,0.22),0_5px_10px_rgba(20,30,45,0.33)] transition-transform duration-200 ${
        checked ? 'translate-x-[33px]' : 'translate-x-0'
      }`}
    >
      <span
        className={`absolute left-1/2 top-1/2 h-[4.5px] w-[4.5px] -translate-x-1/2 -translate-y-1/2 rounded-full ${
          checked
            ? 'bg-[#51ee87] shadow-[0_0_6px_rgba(81,238,135,0.82)]'
            : 'bg-[#77808b]'
        }`}
      />
    </span>
