...
import { Eye, EyeOff, LockKeyhole, UnlockKeyhole, ChevronUp, ChevronDown, Copy, Trash2, Group, Ungroup, ImagePlus, Crop, Type, Shapes, Image as ImageIcon, Layers } from 'lucide-react';
...
import inspector from './LogoEditorInspector.module.css';
...
export type LogoEditorPropertiesProps = {
...
/** Layer management stays separate from the selected layer's contextual controls. */
export function LogoLayersPanel({ project, selected, onSelect, onChange, onDuplicate, onDelete, onGroup, onUngroup, onReorder }: LogoEditorPropertiesProps) {
  ...
  return <aside className={styles.layersPanel} aria-label="Sloji logotipa">
    <div className={styles.panelHeading}><h2>Sloji</h2><span>{project.layers.length}</span></div>
    <div className={styles.layerTree} data-testid="logo-layers">{tree(project.layers)}{!project.layers.length && <p className={styles.hint}>Dodajte sliko, besedilo ali obliko z orodno vrstico.</p>}</div>
    <div className={inspector.layerPrimaryAction}><button type="button" className={styles.button} aria-label="Podvoji izbrane sloje" disabled={!selected.length} onClick={onDuplicate}><Copy />Podvoji izbor</button></div>
    <div className={styles.layerActions}>
      <button type="button" title="Premakni naprej" aria-label="Premakni sloj naprej" disabled={!layer} onClick={() => onReorder(1)}><ChevronUp /></button>
      <button type="button" title="Premakni nazaj" aria-label="Premakni sloj nazaj" disabled={!layer} onClick={() => onReorder(-1)}><ChevronDown /></button>
      <button type="button" title="Združi v skupino" aria-label="Združi v skupino" disabled={selected.length < 2} onClick={onGroup}><Group /></button>
      <button type="button" title="Razdruži skupino" aria-label="Razdruži skupino" disabled={layer?.type !== 'group'} onClick={onUngroup}><Ungroup /></button>
      <button type="button" title="Izbriši sloje" aria-label="Izbriši sloje" disabled={!selected.length} onClick={onDelete}><Trash2 /></button>
    </div>
  </aside>;
}

export default function LogoEditorProperties({ project, assets, selected, onError, onChange, onReplaceImage, onCropImage, keepRatio, setKeepRatio }: LogoEditorPropertiesProps) {
  const layer = selected.length === 1 ? findLogoLayer(project, selected[0]) : undefined;
  ...
  return <aside className={styles.inspector} aria-label="Lastnosti logotipa" data-testid="logo-properties" data-logo-inspector-layer={layer?.id}>
    <div className={styles.panelHeading}><h2>Lastnosti</h2><span>{selected.length > 1 ? 'Izbranih slojev: ' + selected.length : layer?.type === 'image' ? 'Slika' : layer?.type === 'text' ? 'Besedilo' : layer?.type === 'shape' ? 'Oblika' : layer?.type === 'group' ? 'Skupina' : 'Platno'}</span></div>
    <div className={styles.properties} key={selected.join(':')}>
      {!selected.length ? <>
        <div className={inspector.sectionHeading}><h3>Velikost platna</h3><span>px</span></div>
        <div className={styles.twoColumns}><LogoNumber label="Širina platna" value={project.canvas.width} min={1} max={8192} onChange={width => onChange({ ...project, canvas: { ...project.canvas, width } })} /><LogoNumber label="Višina platna" value={project.canvas.height} min={1} max={8192} onChange={height => onChange({ ...project, canvas: { ...project.canvas, height } })} /></div>
        <p className={styles.hint}>Platno je prosojno. Ozadje predogleda se ne izvozi. Največ 4 milijone slikovnih pik.</p>
        <p className={inspector.emptyHint}>Izberite sloj na platnu ali levo v seznamu. Njegove nastavitve se prikažejo tukaj.</p>
      </> : selected.length > 1 ? <>
        <p className={styles.hint}>Premikajte in spreminjajte velikost izbranih slojev skupaj. Poravnava je nad platnom, združevanje in podvajanje pa pod seznamom slojev.</p>
        <LogoNumber label="Prosojnost izbora (%)" value={Math.round((findLogoLayer(project, selected[0])?.opacity ?? 1) * 100)} min={0} max={100} onChange={value => change(item => { if (!item.locked) item.opacity = value / 100; })} />
      </> : layer && <>
        <label className={styles.field}><span>Ime sloja</span><input className={styles.input} aria-label="Ime sloja" data-logo-layer-name={layer.id} value={layer.name} maxLength={120} onChange={event => change(item => { item.name = event.target.value; })} /></label>
        ...
        <fieldset disabled={locked} className={styles.propertyFields}>
              {layer.type === 'text' && <>
                <label className={styles.field}><span>Besedilo</span><textarea rows={3} className={styles.input} aria-label="Vsebina besedila" data-logo-text-editor={layer.id} value={layer.text} maxLength={2000} onChange={event => change(item => { if (item.type === 'text') item.text = event.target.value; })} /></label>
                ...
                <details className={inspector.advanced}><summary>Razmik in poravnava</summary><div className={inspector.advancedBody}>
          ...
          {layer.type === 'image' && <>
            <div className={inspector.imageActions}>
              <button type="button" className={styles.primary} onClick={() => onCropImage(layer.id)}><Crop />Izreži sliko</button>
              <button type="button" className={styles.button} onClick={() => onReplaceImage(layer.id)}><ImagePlus />Zamenjaj sliko</button>
            </div>
            <p className={styles.hint}>Slika je en sloj. Vdelano besedilo ni ločeno urejljivo; dodate ga lahko kot nov besedilni sloj. Dvoklik na sliko odpre izrez.</p>
          </>}
          {layer.type === 'group' && <p className={styles.hint}>Premik in velikost veljata za celotno skupino. Posamezen sloj izberite v seznamu levo.</p>}
          <div className={inspector.sectionHeading}><h3>Velikost</h3><span>px</span></div>
          ...
          <details className={inspector.advanced}><summary>Položaj, zasuk in prosojnost</summary><div className={inspector.advancedBody}>
          ...
          {layer.type === 'image' && <details className={inspector.advanced}><summary>Več možnosti slike</summary><div className={inspector.advancedBody}>
