...
export async function GET(request: Request) {
  try {
    ...
    if (typeParam === 'all' || typeParam === 'purchase_order') {
      const attachmentResult = await pool.query<AttachmentRow>(
        `
      ...
      rows.push(...attachmentResult.rows.map((row) => ({ ...row, type: 'purchase_order' })));
    }
    ...
    if (typeParam === 'all' || typeParam !== 'purchase_order') {
      ...
      const docResult = await pool.query<DocRow>(docQuery, docParams);
