  ...
  return (
    <div className="group relative" data-filled={filled ? 'true' : 'false'}>
      <input
        ...
        className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3 pb-1.5 pt-5 text-sm text-slate-900 outline-none transition focus:border-[#ede8fe] focus:ring-1 focus:ring-[#ede8fe] disabled:bg-slate-100 disabled:text-slate-400"
      />
  ...
  return (
    <div className="relative">
      ...
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="h-11 w-full appearance-none rounded-xl border border-slate-300 bg-white px-3 pb-1.5 pt-5 text-sm text-slate-900 outline-none transition focus:border-[#ede8fe] focus:ring-1 focus:ring-[#ede8fe]"
      >
        {children}
      </select>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border p-3 shadow-sm" style={{ background: "linear-gradient(180deg, rgba(250,251,252,0.96) 0%, rgba(242,244,247,0.96) 100%)", borderColor: "#e2e8f0", boxShadow: "0 10px 24px rgba(15,23,42,0.06)" }}>
        <div className="mt-3 grid gap-2 md:grid-cols-[minmax(280px,1fr)_220px_auto_auto] md:items-center">
          <input
            value={search}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Poišči po nazivu, SKU ali kategoriji …"
            className="h-8 rounded-xl border border-slate-300 px-3 text-xs focus:border-[#ede8fe] focus:ring-1 focus:ring-[#ede8fe]"
          />
          <select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)} className="h-8 rounded-xl border border-slate-300 px-3 text-xs focus:border-[#ede8fe] focus:ring-1 focus:ring-[#ede8fe]">
          ...
          <button
            ...
            className="h-8 rounded-xl border border-amber-500 bg-amber-200 px-3 text-xs font-semibold text-amber-950 transition hover:bg-amber-300 disabled:pointer-events-none disabled:opacity-45"
          >
            Arhiviraj
          </button>
        ...
        <div className="mt-2 inline-flex h-8 items-center gap-1 rounded-full border border-slate-300 bg-white px-1">
          {statusTabs.map((tab) => {
          ...
          <div className="h-full w-full max-w-md overflow-y-auto border-l border-slate-200 bg-white p-4 shadow-xl">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="group relative" data-filled={draft.description ? 'true' : 'false'}>
                <textarea value={draft.description} onChange={(event) => setDraft((prev) => ({ ...prev, description: event.target.value }))} placeholder=" " className="min-h-[90px] w-full rounded-xl border border-slate-300 bg-white px-3 pb-2 pt-5 text-sm text-slate-900 outline-none transition focus:border-[#ede8fe] focus:ring-1 focus:ring-[#ede8fe]" />
