...
function FloatingInput({
  ...
  return (
    <div className="group relative" data-filled={filled ? 'true' : 'false'}>
      ...
      />
      <label className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 bg-white px-0 text-xs text-slate-400 transition-all duration-150 group-focus-within:top-1.5 group-focus-within:translate-y-0 group-focus-within:px-1 group-focus-within:text-[10px] group-focus-within:text-slate-600 group-data-[filled=true]:top-1.5 group-data-[filled=true]:translate-y-0 group-data-[filled=true]:px-1 group-data-[filled=true]:text-[10px] group-data-[filled=true]:text-slate-600">
        {label}
      </label>
    </div>
  );
...
export default function AdminItemsManager({ seedItems }: { seedItems: Item[] }) {
  ...
  const filteredItems = useMemo(() => {
    ...
    next.sort((a, b) => {
      ...
      else if (sortKey === 'status') cmp = textCmp(a.active ? 'Aktiven' : 'Arhiviran', b.active ? 'Aktiven' : 'Arhiviran');
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        <div className="grid gap-2 md:grid-cols-3">
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Poišči po nazivu, SKU ali kategoriji …" className="h-10 rounded-lg border border-slate-300 px-3 text-sm" />
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase text-slate-500">
              <tr>
                ...
                <th className="px-3 py-2">Trenutna cena</th>
                <th className="px-3 py-2">
            ...
            <tbody>
              {filteredItems.map((item) => (
                <tr key={item.id} className="border-t border-slate-200">
                  ...
                  <td className="px-3 py-2 text-right"><div className="flex items-center justify-end gap-2"><button type="button" onClick={() => openEdit(item)} className="rounded-md border border-slate-200 px-2 py-1 text-xs text-slate-700 hover:bg-slate-50">Uredi</button><button type="button" onClick={() => duplicate(item)} className="rounded-md border border-slate-200 px-2 py-1 text-xs text-slate-700 hover:bg-slate-50">Podvoji</button><button type="button" onClick={() => archive(item)} className="rounded-md border border-amber-200 px-2 py-1 text-xs text-amber-700 hover:bg-amber-50">Arhiviraj</button><button type="button" onClick={() => remove(item)} className="rounded-md border border-rose-200 px-2 py-1 text-xs text-rose-700 hover:bg-rose-50">Izbriši</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
