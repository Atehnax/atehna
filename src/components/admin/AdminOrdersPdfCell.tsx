            ...
            {versions.map((documentOption, index) => (
              <li key={`${button.key}-${documentOption.blob_url}-${documentOption.created_at}-${index}`}>
                <a
                  ...
                  className="flex items-center justify-between rounded-md border border-transparent bg-white px-2 py-1 text-[10px] text-slate-700 transition hover:border-slate-200 hover:bg-[#ede8ff]"
        ...
        return (
          <div key={button.key} className="relative" data-no-row-nav>
            <button
              ...
              className="relative inline-flex h-6 items-center rounded-md border border-slate-200 bg-slate-50 px-1.5 py-1 text-[11px] font-medium text-slate-700 transition hover:bg-[#ede8ff] disabled:cursor-not-allowed disabled:text-slate-300"
            >
