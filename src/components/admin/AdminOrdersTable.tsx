...
export default function AdminOrdersTable({
  ...
  useEffect(() => {
    if (!isStatusHeaderMenuOpen) return;

    const handleOutsideClick = (event: MouseEvent) => {
      if (!statusHeaderMenuRef.current?.contains(event.target as Node)) {
        setIsStatusHeaderMenuOpen(false);
      }
    };

    const handleEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setIsStatusHeaderMenuOpen(false);
    };
  ...
  useEffect(() => {
    if (selectedCount === 0) setIsStatusHeaderMenuOpen(false);
  }, [selectedCount]);
