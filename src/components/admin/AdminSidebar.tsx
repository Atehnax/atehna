...
export default function AdminSidebar() {
  ...
  return (
    <aside className={`h-full self-stretch shrink-0 overflow-y-auto transition-all duration-300 ${isCollapsed ? 'w-10' : 'w-[19rem]'}`}>
      <div className="relative h-full overflow-visible border-r border-[color:var(--semantic-info-border)] bg-[color:var(--blue-50)] shadow-sm">
        <button
          type="button"
          aria-label={isCollapsed ? 'Odpri meni' : 'Skrij meni'}
          onClick={() => setIsCollapsed((current) => !current)}
          className="absolute right-[-14px] top-3 z-20 inline-flex h-8 w-8 items-center justify-center rounded-xl border border-[color:var(--semantic-info-border)] bg-white text-sm text-[color:var(--semantic-info)] shadow-sm transition hover:bg-[color:var(--blue-100)]"
        >
                  ...
                  <div key={link.href}>
                    <Link
                      ...
                      className={`flex items-center gap-2 rounded-xl px-2.5 py-2 pr-4 text-sm transition ${
                        isActive
                          ? 'bg-white/85 font-semibold text-[color:var(--blue-700)]'
