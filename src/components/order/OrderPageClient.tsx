...
export default function OrderPageClient() {
  ...
  const mailtoLink = useMemo(() => {
    ...
    const subject = `Naročilo – ${formData.firstName} ${formData.lastName} – ${formData.reference}`;
                  ...
                  {'logos' in method && method.logos && (
                    <span className="flex flex-wrap gap-2">
                      {method.logos.map((logo) => (
                        <span
                          key={logo.src}
                          className="flex items-center rounded-lg border border-slate-200 bg-white px-2 py-1"
                        >
                          <Image src={logo.src} alt={logo.alt} width={56} height={32} />
                        </span>
                      ))}
                    </span>
                  )}
