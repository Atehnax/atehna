...
import type { BusinessAnalyticsResponse, BusinessFilters, CanonicalOrder } from './businessAnalytics';
...
export function buildBusinessOrderPreview(allOrders: CanonicalOrder[], asOf = new Date(), filters: Pick<BusinessFilters, 'entrySource' | 'history'> = {}): BusinessOrderPreview {
  ...
  const summary = (period: BusinessPeriod) => projectBusinessOrderSummary(aggregateBusinessAnalytics({ allOrders, quotes: [], filters: { range: period.range, customerType: 'all', status: 'all', source: 'all', ...filters }, period, asOf: asOf.toISOString() }).summary);
  const firstObserved = allOrders.map(order => order.submittedAt).sort()[0];
  const query = new URLSearchParams({ view: 'narocila', range: '90D', asOf: asOf.toISOString(), entrySource: filters.entrySource ?? 'all', history: filters.history ?? 'all' });
