...
async function readOrders(client: PoolClient, asOf: Date, includeDetails = false): Promise<CanonicalOrder[]> { const result = await client.query(canonicalOrdersSql(includeDetails), [asOf.toISOString()]); return result.rows.map(mapCanonicalOrder); }
async function readQuotes(client: PoolClient, asOf: Date): Promise<CanonicalQuote[]> { const result = await client.query(`
  with first_issue as (
