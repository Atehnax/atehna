...
export async function fetchWebsiteAnalytics(options?: {
  ...
  return instrumentCatalogLoader('fetchWebsiteAnalytics', '/admin/analitika/splet', async () => {
    ...
    const [visitsByDayResult, topPagesResult, topProductsResult, retentionResult, retentionByDayResult] = await profileRoutePhase(
      'db',
      'fetchWebsiteAnalytics:queries',
      () =>
        Promise.all([
          pool.query(
            `
    ...
    profilePayloadEstimate('fetchWebsiteAnalytics:summary', summary);
    return summary;
  });
}
