  ...
  if (typeof window === 'undefined') {
    return (
      <div aria-label="Paginacija tabele" className={`inline-flex items-center gap-2 ${className ?? ''}`.trim()}>
        ...
        <PageSizeSelect value={itemsPerPage} options={itemsPerPageOptions} onChange={onChangeItemsPerPage} className="[&_button]:!h-7 [&_button]:!w-[42px]" />
  ...
  return (
    <div aria-label="Paginacija tabele" className={`admin-orders-pagination inline-flex items-center gap-2 ${className ?? ''}`.trim()}>
      ...
      <PageSizeSelect
        ...
        className="[&_button]:!h-7 [&_button]:!w-[42px]"
      />
      <ElasticPagination
