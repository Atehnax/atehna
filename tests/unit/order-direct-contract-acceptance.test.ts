...
test('only an ordinary pending direct order is implicitly accepted for processing', () => {
  ...
  const ineligible: DirectOrderSellerAcceptanceTransition[] = [
    ...
    { ...eligibleTransition, commitmentStatus: 'pending_confirmation' },
...
test('status route persists acceptance before status effects in one transaction', () => {
  ...
  const acceptanceIndex = route.indexOf("set contract_status = 'accepted'");
  const acceptanceEndIndex = route.indexOf('returning contract_status', acceptanceIndex);
  ...
  assert.match(acceptanceSql, /commitment_status = 'binding'/u);
  ...
  assert.match(route, /contract_accepted_automatically: automaticallyAcceptsDirectOrder/u);
  assert.match(route, /contract_accepted_while_draft:[\s\S]*?workflow\?\.is_draft === true/u);
  assert.match(route, /else if \(automaticallyAcceptsDirectOrder\) \{[\s\S]*?shouldEnqueueStatusEmail = false/u);
